...
export default function Sidebar() {
  ...
  const menuItems = [
    ...
    { name: "Simulator Jaringan", path: "/simulation", icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>) },
    { name: "Game Krimp RJ45", path: "/game-crimping", icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>) },
    ...
    <>
      ...
      <div className="lg:hidden fixed top-3 left-3 z-50">
        <button
          ...
          className="p-2.5 bg-white/90 backdrop-blur-md text-slate-700 rounded-2xl shadow-lg border border-slate-200/60 focus:outline-none active:scale-95 transition-transform"
        >
          {isOpen ? (
      ...
      {/* Sidebar Container */}
      <div
        className={`fixed lg:sticky top-0 left-0 z-40 w-[270px] bg-white/95 backdrop-blur-xl text-slate-800 min-h-screen p-5 flex flex-col justify-between border-r border-slate-200/60 transition-transform duration-300 transform lg:transform-none ${
      ...
      >
        <div className="space-y-5 pt-12 lg:pt-0">
          ...
          <div className="flex items-center space-x-3 px-2 pt-2">
            <div className="bg-gradient-to-br from-indigo-500 to-violet-600 p-2.5 rounded-2xl shadow-lg shadow-indigo-500/25">
              <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z" />
              </svg>
            </div>
            <div>
              <span className="text-xl font-black tracking-tight text-gradient">SIM-TKJ</span>
              <p className="text-[9px] text-slate-400 uppercase tracking-[0.2em] font-bold">Virtual Lab SMK</p>
            </div>
          </div>
              ...
              <div className="flex items-center space-x-3 mb-3">
                ...
                <div className="overflow-hidden flex-1">
                  ...
                  <div className="flex items-center space-x-1.5 mt-0.5">
                    <span className="text-[10px] bg-indigo-500 text-white font-bold px-2 py-0.5 rounded-full shadow-sm">
                      Lvl {userData.level}
                    </span>
                    <span className="text-[10px] text-indigo-600 font-bold">{userData.xp} XP</span>
                  </div>
                </div>
              </div>
          ...
          {userData.role === "teacher" && (
            <div className="bg-gradient-to-br from-purple-50 to-fuchsia-50 border border-purple-100/80 rounded-2xl p-4 flex items-center space-x-3">
              ...
              <div className="overflow-hidden">
                <p className="font-bold text-slate-800 text-sm truncate">{userData.name}</p>
                <span className="text-[10px] bg-purple-500 text-white font-bold px-2.5 py-0.5 rounded-full shadow-sm">
                  Guru Pengampu
                </span>
              </div>
            </div>
            ...
            {menuToRender.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center space-x-3 px-3.5 py-2.5 rounded-xl transition-all duration-200 group font-semibold text-[13px] ${
                    isActive
      ...
      {isOpen && (
        <div
          onClick={() => setIsOpen(false)}
          className="fixed inset-0 bg-slate-950/30 z-30 lg:hidden backdrop-blur-sm"
        />
      )}
    </>
  );
